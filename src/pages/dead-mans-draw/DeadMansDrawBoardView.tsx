...
export function DeadMansDrawBoardView({
  ...
  const isLastTutorialStep = tutorialOpen && typeof tutorialStep === "number"
  ...
  const getFocusElement = () => {
    ...
    switch (currentTutorialStep.focus) {
      ...
      case "deck-section": return "deck-section";
      case "player-panel": return "player-panel";
      case "treasure-area": return "treasure-area";
      case "cards": return null;
      case "powers": return null;
      default: return null;
    }
  };
  ...
  const activeEffectCard = pendingEffect
    ? visibleTreasureArea.find((card) => card.id === pendingEffect.sourceCardId) ?? visibleTreasureArea[visibleTreasureArea.length - 1] ?? null
    : null;
  ...
  const tutorialCardPositionClass = focusElement === "treasure-area"
    ? "top-4"
    : focusElement
      ? "bottom-4"
      : "top-4";
  ...
  return (
    <div dir={dir} className="relative min-h-screen overflow-hidden text-white">
          ...
          >
            <div className="mb-4 flex items-center justify-between gap-3">
              <span className="rounded-full border border-amber-300/60 bg-amber-400/15 px-3 py-1 text-xs font-bold uppercase tracking-[0.24em] text-amber-100">
                {t("tutorialStep", { current: (tutorialStep ?? 0) + 1, total: tutorialSteps?.length ?? 0 })}
              </span>
              <button
                type="button"
            ...
            <h3 className="font-cinzel text-xl font-bold text-amber-100">{currentTutorialStep.title}</h3>
            <p className="mt-3 text-sm leading-6 text-slate-100/90">{currentTutorialStep.description}</p>
            {focusElement ? (
              <p className="mt-3 text-xs leading-5 text-amber-100/80">
                {dir === "rtl" ? "کادر طلایی روی همان بخش نشان می‌دهد الان دربارهٔ کدام قسمت صحبت می‌کنیم." : "The glowing gold frame shows the exact section being explained right now."}
              </p>
            ) : null}
            <div className="mt-5 flex items-center justify-between gap-3">
              ...
              >
                {isLastTutorialStep ? (dir === "rtl" ? "پایان آموزش" : "Finish walkthrough") : t("tutorialNext")}
              </button>
            </div>
          </motion.div>
        </>
        ...
        <div className="space-y-4 pb-8">
          ...
          {currentState.players[1] ? (
            <motion.div
              ...
              animate={focusElement === "player-panel" ? {
                borderColor: ["rgba(251, 191, 36, 0.3)", "rgba(251, 191, 36, 0.8)", "rgba(251, 191, 36, 0.3)"],
                boxShadow: ["0 0 0 3px rgba(251, 191, 36, 0.1)", "0 0 0 3px rgba(251, 191, 36, 0.4)", "0 0 0 3px rgba(251, 191, 36, 0.1)"],
              } : {}}
              transition={{ duration: 2, repeat: Infinity }}
              className={cn("rounded-2xl border-2 border-transparent", focusElement === "player-panel" && highlightedSectionClass)}
            >
              <PlayerStack
          ...
          <motion.div
            ...
            animate={focusElement === "deck-section" ? {
              borderColor: ["rgba(251, 191, 36, 0.3)", "rgba(251, 191, 36, 0.8)", "rgba(251, 191, 36, 0.3)"],
              boxShadow: ["0 0 0 3px rgba(251, 191, 36, 0.1)", "0 0 0 3px rgba(251, 191, 36, 0.4)", "0 0 0 3px rgba(251, 191, 36, 0.1)"],
            } : {}}
            transition={{ duration: 2, repeat: Infinity }}
            className={cn("relative rounded-2xl border-2 border-transparent", focusElement === "deck-section" && highlightedSectionClass)}
          >
            ...
            <div className="absolute right-0 top-0">
              <DeckCounter label={t("deadMansDrawBurnPile")} count={currentState.discardPile.length} deckId="discard" />
            </div>
          ...
          <motion.div
            ...
            animate={focusElement === "treasure-area" ? {
              borderColor: ["rgba(251, 191, 36, 0.3)", "rgba(251, 191, 36, 0.8)", "rgba(251, 191, 36, 0.3)"],
              boxShadow: ["0 0 0 3px rgba(251, 191, 36, 0.1)", "0 0 0 3px rgba(251, 191, 36, 0.4)", "0 0 0 3px rgba(251, 191, 36, 0.1)"],
            } : {}}
            transition={{ duration: 2, repeat: Infinity }}
            className={cn("rounded-[30px] border-2 border-transparent bg-slate-950/48 p-4 shadow-[0_18px_55px_rgba(2,6,23,0.35)]", focusElement === "treasure-area" && highlightedSectionClass)}
          >
            ...
            <div className="mt-4 grid grid-cols-3 gap-2" data-dead-draw-treasure-grid="true">
              {visibleTreasureArea.length ? visibleTreasureArea.map((card, index) => (
                <div key={card.id} className="relative flex justify-center" data-dead-draw-treasure-card={card.id}>
                  <motion.div
                    initial={false}
                    animate={{
          ...
          <motion.div
            ...
            transition={{ duration: 2, repeat: Infinity }}
            className={cn("space-y-3 rounded-2xl border-2 border-transparent", focusElement === "player-panel" && highlightedSectionClass)}
          >
      ...
      {decisionOverlayOpen && guideCopy && pendingEffect.kind === "misfire" ? (
        <>
          ...
          <div className="pointer-events-none fixed inset-x-0 top-[11vh] z-[50] flex justify-center px-4">
            <div className="max-w-md rounded-[28px] border border-amber-300/40 bg-[linear-gradient(180deg,rgba(18,12,6,0.92),rgba(9,8,12,0.88))] px-5 py-3 text-center shadow-[0_18px_40px_rgba(0,0,0,0.45)] backdrop-blur-md">
              <p className="font-cinzel text-xs uppercase tracking-[0.32em] text-amber-200/85">{guideCopy.title}</p>
              <p className="mt-2 text-sm leading-6 text-amber-50/90">{guideCopy.body}</p>
            </div>
          </div>
          <div className="pointer-events-none fixed inset-x-0 top-[24vh] z-[51] flex justify-center px-4">
            <div className="rounded-[28px] border border-rose-300/50 bg-rose-950/55 px-6 py-5 shadow-[0_0_24px_rgba(248,113,113,0.35)]">
              <img src={SUIT_IMAGES.pistol} alt="Misfire" className="mx-auto h-16 w-16 rounded-full object-cover" />
            </div>
          </div>
            ...
            <div className="pointer-events-auto w-full max-w-lg rounded-[28px] border border-white/12 bg-[linear-gradient(180deg,rgba(10,16,28,0.92),rgba(5,8,18,0.94))] px-4 py-4 shadow-[0_24px_50px_rgba(2,6,23,0.55)] backdrop-blur-md">
              <div className="space-y-3">
                <div className="flex flex-wrap justify-center gap-3">
                  {pendingEffect.kind === "misfire" && pendingEffect.options.map((card) => (
                    <button
                      key={card.id}
                      type="button"
      ...
      {decisionOverlayOpen && (pendingEffect.kind === "astrolabe" || pendingEffect.kind === "map") ? (
        <div className="fixed inset-x-0 z-[52] flex justify-center px-4" style={{ top: "28%" }}>
          <div className="pointer-events-auto w-full max-w-lg rounded-[28px] border border-white/12 bg-[linear-gradient(180deg,rgba(10,16,28,0.92),rgba(5,8,18,0.94))] px-4 py-4 shadow-[0_24px_50px_rgba(2,6,23,0.55)] backdrop-blur-md">
            {pendingEffect.kind === "astrolabe" ? (
              <div className="space-y-4">
                <p className="text-center text-xs uppercase tracking-[0.24em] text-teal-100/70 font-cinzel">
                  {dir === "rtl" ? "کارت‌های قابل دیدن" : "Visible Cards"}
                </p>
            ...
            {pendingEffect.kind === "map" ? (
              <div className="space-y-3">
                <p className="text-center text-xs uppercase tracking-[0.24em] text-amber-100/70 font-cinzel">
                  {dir === "rtl" ? "کارت سوخته را انتخاب کنید" : "Choose a Burned Card"}
                </p>
                <div className="flex flex-wrap justify-center gap-3">
      ...
      {cardFlights.map((flight) => (
        <motion.div
          ...
          animate={{
            ...
            scale: flight.kind === "burn" ? [1, 1.04, 0.84] : [flight.kind === "reveal" ? 0.86 : 1, 1.03, 0.94],
            opacity: [0.98, 1, 0.82],
            rotate: flight.kind === "burn" ? [0, 6, 18] : [-4, 0, 2],
          }}
          transition={{ duration: flight.durationMs / 1000, ease: "easeInOut" }}
        >
          <CardChip card={flight.card} compact highlighted={flight.kind === "burn"} isBusting={flight.kind === "burn"} />
        </motion.div>
      ))}
    </div>
  );
